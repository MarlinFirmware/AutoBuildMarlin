...
// Return an array containing dicts. Each entry is one architecture.
// Each architecture contains a list of boards with: a (meaningless) ID number, name, and description.
//
// The data out format is:
// [archindex] = { arch: 'Arch Section', boards: [ {  id: 'board_id', name: 'Board Name', desc: 'This is a board' }, ... ] }
//
var boards_list;
function extractBoardsList() {
  ...
  while (r = findAll.exec(files.boards.text)) {
    if (r[1]) {
      ...
      out[++ind] = { arch: r[1], boards: [] };
    }
    else {
      // Found a board. Add it to the current group.
      out[ind].boards.push({ id: r[3], name: r[2], desc: r[4] });
    }
  }
